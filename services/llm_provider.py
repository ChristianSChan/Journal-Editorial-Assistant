...
def llm_provider() -> str:
    provider = os.getenv("LLM_PROVIDER", "codex_cli").strip().casefold()
    return provider if provider in {"openai", "local_cli", "codex_cli"} else "codex_cli"
...
def codex_cli_home() -> str:
    return os.getenv("CODEX_CLI_HOME", DEFAULT_CODEX_HOME).strip()
...
def _call_openai_text(system_prompt: str, user_payload: Any, temperature: float = 0) -> str | None:
    ...
    try:
        ...
        _set_last_llm_error("OpenAI-compatible API request failed or returned an unexpected response.")
        return None
